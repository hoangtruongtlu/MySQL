...
-- hoặc  trả về user có username chứa chuỗi của người dùng nhập vào 
 ...
 BEGIN
		IF In_Select = 1 THEN
			SELECT *
			FROM `Group`
			WHERE GroupName LIKE StringInput;
        ELSE
			SELECT AccountID,UserName,FullName
            FROM `Account`
            WHERE UserName LIKE StringInput;
		END IF;
END $$
DELIMITER ;

CALL Name_GrOrUser('Truong',2);
SELECT * FROM `Account`;
SELECT * FROM `Group`;
...
BEGIN
		SELECT Recent6Month.`Month`,Count(QuestionID)  Count
        FROM (
				SELECT MONTH(DATE_SUB(CURRENT_DATE(),INTERVAL 0 MONTH)) `Month`
                UNION
                SELECT MONTH(DATE_SUB(CURRENT_DATE(),INTERVAL 1 MONTH)) `Month`
                UNION
                SELECT MONTH(DATE_SUB(CURRENT_DATE(),INTERVAL 2 MONTH)) `Month`
                UNION
                SELECT MONTH(DATE_SUB(CURRENT_DATE(),INTERVAL 3 MONTH)) `Month`
                UNION
                SELECT MONTH(DATE_SUB(CURRENT_DATE(),INTERVAL 4 MONTH)) `Month`
                UNION
                SELECT MONTH(DATE_SUB(CURRENT_DATE(),INTERVAL 5 MONTH)) `Month`
			) AS Recent6Month
